...
async def create_per_chan(member):
	...
	embed = discord.Embed(title="Welcome to Your New Personal Channel!", description=f"**(En)**Welcome to your new personal channel, <@{member.id}>. Your Personal Channel in our server. You have your full control over your personal channel (but do not violate [discord's TOS](https://discord.com/terms) or server rules).\n**(ES)**Bienvenido a su nuevo canal personal, <@{member.id}>. Tu canal personal en nuestro servidor. Tienes el control total sobre tu canal personal (pero no infrinjas los [TOS de Discord](https://discord.com/terms) o las reglas del servidor).\n**(Ru)**Добро пожаловать на ваш новый личный канал, <@{member.id}>. Ваш личный канал на нашем сервере. Вы полностью контролируете свой личный канал (но не нарушаете [TOS или правила сервера Discord](https://discord.com/terms)).", color=0x88ff4b)
	embed.add_field(name='Rename this Channel ', value="**(En)**This is your personal channel so of course you can rename it! If you're on Desktop then click on the settings icon (next to your channel's name in channel list) and if you're on a mobile device, hold the channel to go the edit-channel page of your channel. Your channel deserves a proper title.\n**(Es)**Este es tu canal personal, así que, por supuesto, ¡puedes cambiarle el nombre! Si está en el escritorio, haga clic en el ícono de configuración (junto al nombre de su canal en la lista de canales) y si está en un dispositivo móvil, mantenga presionado el canal para ir a la página de edición del canal de su canal. Tu canal merece un título adecuado.\n**(Ru)**Это ваш личный канал, поэтому, конечно, вы можете его переименовать! Если вы находитесь на рабочем столе, щелкните значок настроек (рядом с названием вашего канала в списке каналов), а если вы используете мобильное устройство, удерживайте канал, чтобы перейти на страницу редактирования канала вашего канала. Ваш канал заслуживает надлежащего названия.", inline=False)
	embed.add_field(name='Change the Channel\'s Description', value="**(En)**You can change the description of your channel in the same page you can change the name of your channel.\n**(Es)**Puedes cambiar la descripción de tu canal en la misma página puedes cambiar el nombre de tu canal.\n**(Ru)**Вы можете изменить описание вашего канала на той же странице, вы можете изменить название вашего канала.", inline=False)
	embed.add_field(name='Delete any message', value="**(En)**Are there messages in your channel that you don't want it to have? Clean your channel up.\n**(Es)**¿Hay mensajes en tu canal que no quieres que tenga? Limpia tu canal.\n**(Ru)**Есть ли на вашем канале сообщения, которые вам не нужны? Очистите свой канал.", inline=False)
	embed.add_field(name='Pin any message', value="**(En)**You can pin any message in your channel. On desktop, hold shift and click the pin icon or click the three-dots icon and pin. On Mobile, hold the message and then click on the pin option.\n**(Es)**Puedes anclar cualquier mensaje en tu canal. En el escritorio, mantenga presionada la tecla Mayús y haga clic en el ícono de alfiler o haga clic en el ícono de tres puntos y el alfiler. En el dispositivo móvil, mantenga presionado el mensaje y luego haga clic en la opción de pin.\n**(Ru)**Вы можете закрепить любое сообщение в своем канале. На рабочем столе, удерживая Shift, щелкните значок булавки или щелкните значок с тремя точками и закрепите. На мобильном устройстве удерживайте сообщение, а затем нажмите на значок булавки.", inline=False)
	embed.add_field(name='Manage permissions', value="**(En)**You can manage permissions for members for your channel.\n**(Es)**Puede administrar los permisos de los miembros de su canal.\n**(Ru)**Вы можете управлять разрешениями для участников вашего канала.", inline=False)
	embed.set_footer(text="Have Fun! ¡Divertirse! Повеселись!")
